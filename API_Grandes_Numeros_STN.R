...
function(){
  ...
  if (!file.exists(file_periodo)){
    ...
    anim_save(file_periodo, animation = last_animation())
  }
...
function(){
  ...
  series_temporais<-
  rtn::get_12_month_accumulated_account_data_by_month(.data=c("Bolsa família",
  ...
  if (!file.exists(file_periodo)){
    ...
    animate(gif_linhas_desp, nframes = (round(nrow(series_temporais)/6)), height = 488, width = 668,
            renderer = gifski_renderer(loop = FALSE))
